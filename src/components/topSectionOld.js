...
Try Different browser or Install Metamask.`);
      ...
      const accounts = await web3.eth.getAccounts();
        let result = await Contract.methods.getStakedTokenOfUser(accounts[0]).call()
...
Try Different browser or Install Metamask.`);
      ...
      const accounts = await web3.eth.getAccounts();
        let result = await Contract.methods.getUnStakedTokenOfUser(accounts[0]).call()
        // await Contract.methods.getStakedTokenOfUser(count).send({
        //   from: accounts[0],
        //   value: web3.utils.toWei(`${count * price}`, "ether"),
        // });
...
Try Different browser or Install Metamask.`);
      ...
      const accounts = await web3.eth.getAccounts();
        await Contract.methods.ClaimYourStakedMultipleToken(idsForUnstaking).send({
          from: accounts[0],
          // value: web3.utils.toWei(),
        });
        setStakeIdsLoading(false);
  ...
  const showMessageUN = () => {
    ...
    if(abc)
    return <h4 style={{margin: "0 10px"}}>No token found</h4>
  }
          ...
          <Typography component="div" style={{ maxWidth: 700, padding: 20 }}>
            ...
            <Typography>
              ...
              <>
                {unstakedTokenIds.map((id, i) => {
                    if(id !== "999999999"){
                return <Button  color="inherit" style={{ backgroundColor: checkIdsForStaking(id) ? "#040b38" : "#1e2790"}} variant="contained" //
                className={classes.tokenIds} key={i} onClick={() => setTokenIdForStaking(id)} >{id}</Button>}
                    })}
                    {getStakedPressed && unstakedTokenIds.length < 1 && <h4 style={{margin: "0 10px"}}>No token found</h4>}
                    {showMessage()}
              </>
              }
              </Typography>
            ...
            <Button
              ...
              {stakedTokenIds.map((id, i) => {
                if(id !== "999999999"){
                return <Button  color="inherit" style={{ backgroundColor: checkIdsForUnstaking(id) ? "#040b38" : "#1e2790"}} variant="contained" //
                className={classes.tokenIds} key={i}   onClick={() => setTokenIdForUnstaking(id)} >{id}</Button>}
              })}
               {getUnstakedPressed && stakedTokenIds.length < 1 && <h4 style={{margin: "0 10px"}}>No token found</h4>}
               {showMessageUN()}
              </>
              }
              </Typography>
